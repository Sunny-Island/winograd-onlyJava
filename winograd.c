...
static inline void pad_get_tiles(int x, int y, int lenX, int lenY, int nrows, const float *dataSrc,
    ...
    int i, j;
    for (i = 0; i < lenX; ++i) {
        memcpy(temp + i * 66, dataDst + (x + i) * nrows + y, sizeof(int) * lenY);
        // for (j = 0; j < lenY; ++j) {
...
static void out_transform_4x3_16t(int x, int y, int nrows,
    ...
    _mm512_storeu_ps(dataDst + (x + 2) * nrows + y + 0, bufB);
    _mm512_storeu_ps(dataDst + (x + 2) * nrows + y + 16, bufC);
    _mm512_storeu_ps(dataDst + (x + 2) * nrows + y + 32, bufD);
    _mm512_storeu_ps(dataDst + (x + 2) * nrows + y + 48, bufE);
    ...
    _mm512_storeu_ps(dataDst + (x + 3) * nrows + y + 0, bufB);
    _mm512_storeu_ps(dataDst + (x + 3) * nrows + y + 16, bufC);
    _mm512_storeu_ps(dataDst + (x + 3) * nrows + y + 32, bufD);
    _mm512_storeu_ps(dataDst + (x + 3) * nrows + y + 48, bufE);
